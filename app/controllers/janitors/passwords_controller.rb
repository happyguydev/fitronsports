...
class Janitors::PasswordsController < Devise::PasswordsController
...
=end
  ...
  def edit
    ...
    return redirect_to new_janitor_session_path, notice: 'invalid token' if resource.blank?
    super
  end
  ...
  def update
    ...
    if resource.reset_password(new_password_params[:password], new_password_params[:password_confirmation])
      ...
      redirect_to janitors_root_path, notice: t('devise.passwords.changed_successfully')
    else
